...
#         		print 'hi' 
    ...
    def train(self, trainlen = None, n_job = 1, shared_memory = 'no',verbose='on'):
        ...
        if verbose=='on':
            print
        batchtrain(self,trainlen=trainlen, njob = n_job, phase = 'rough', shared_memory = 'no',verbose=verbose)
        if verbose=='on':
            print
        #######################################
        #Finetuning
        if verbose=='on':
            print
        batchtrain(self,trainlen=trainlen, njob = n_job, phase = 'finetune', shared_memory = 'no',verbose=verbose)
...
def batchtrain(self,trainlen=None, njob = 1, phase = None, shared_memory = 'no', verbose='on'):
    t0 = time()
    nnodes = getattr(self, 'nnodes')
    dlen = getattr(self, 'dlen')
    dim = getattr(self, 'dim')
...
#         trainlen = int(np.ceil(100*mpd))
#         trainlen = 10
#         print 'roughtrain trainlen {}'.format(trainlen)
        #radius for updating
...
#     	print 'finetune trainlen {}'.format(trainlen)
